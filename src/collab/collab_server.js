...
function getRoomCallbacks(roomName) {
  ...
  callbacks.onAddConnection =
    function (data) {
      model.accessPadGlobal(_roomToPadId(roomName), function(pad) {
        // XXX _handlePadUserInfo(pad, data.userInfo);
        pad_events.onUserJoin(pad, data.userInfo);
  ...
  callbacks.onRemoveConnection =
    function (data) {
      model.accessPadGlobal(_roomToPadId(roomName), function(pad) {
        pad_events.onUserLeave(pad, data.userInfo);
      });
    };
...
function _updateDocumentConnectionUserInfo(pad, socketId, userInfo) {
  ...
  if (connectionId) {
    ...
    // XXX _handlePadUserInfo(pad, userInfo);
    // XXX padevents.onUserInfoChange(pad, userInfo);
  ...
  if (msg.type == "USER_CHANGES") {
    ...
    _accessConnectionPad(connection, "CLIENT_MESSAGE", function(pad) {
      ...
      if (payload.authId &&
        ...
        pad_events.onClientMessage(pad, connection.data.userInfo,
                                   payload);
      }
    });
  }
