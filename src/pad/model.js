...
function accessPadGlobal(padId, padFunc, rwMode) {
  ...
  // XXX pad_security.checkAccessControl(padId, rwMode);
  ...
  // XXX pad_migrations.ensureMigrated(padId);
  ...
  return doWithPadLock(padId, function() {
    ...
    meta.status.lastAccess = +new Date();
      ...
      function addRevision(theChangeset, author, optDatestamp) {
        ...
        if (meta.pdlinked && ( ! pad.pdsyncing()) && textChanged) { // XXX avoids infinite loop
          execution.scheduleTask("dbwriter_infreq", "pdsyncDocumentText", 0, [ padId, newRev, theChangeset, author ]);
        }
      }
    ...
    get _meta() { return meta; }
      ...
      try {
        pad_utils.setCurrentPad(padId);
        appjet.requestCache.padsAccessing[padId] = pad;
        return padFunc(pad);
      }
      finally {
    pad_utils.clearCurrentPad();
